...
import { APITestHelpers, TestDataGenerator } from '../../helpers/api-test-helpers';
...
Given('заголовки запроса содержат {string}', async function (this: World, headerString: string) {
...
When('я отправляю GET запрос на {string}', async function (this: World, endpoint: string) {
...
When('я отправляю GET запрос на {string} с параметрами:', async function (this: World, endpoint: string, dataTable) {
  this.startTime = Date.now();
...
When('я отправляю OPTIONS запрос на {string}', async function (this: World, endpoint: string) {
  this.startTime = Date.now();
...
Then('код ответа должен быть {int}', async function (this: World, expectedStatus: number) {
...
Then('каждый товар содержит обязательные поля:', async function (this: World, dataTable) {
...
Then('время ответа не превышает {int} мс', async function (this: World, maxTime: number) {
  expect(this.responseTime).toBeLessThanOrEqual(maxTime);
});

Then('тело ответа содержит товар с ID {string}', async function (this: World, productId: string) {
  const product = this.response.data;
  expect(product.id).toBe(productId);
});

Then('товар содержит полную информацию:', async function (this: World, dataTable) {
  const product = this.response.data;
  const requiredFields = dataTable.hashes();
...
Then('заголовок {string} содержит {string}', async function (this: World, headerName: string, expectedValue: string) {
...
Then('найденные товары содержат слово {string} в названии или описании', async function (this: World, searchTerm: string) {
  const products = this.response.data.data;
...
Then('количество результатов не превышает {int}', async function (this: World, maxCount: number) {
  const products = this.response.data.data;
  expect(products.length).toBeLessThanOrEqual(maxCount);
});

Then('тело ответа содержит пустой список товаров', async function (this: World) {
  const data = this.response.data;
  expect(data.data).toEqual([]);
});

Then('общее количество результатов равно {int}', async function (this: World, expectedTotal: number) {
  const pagination = this.response.data.pagination;
  expect(pagination.total).toBe(expectedTotal);
});

Then('все товары в ответе принадлежат категории {string}', async function (this: World, categoryId: string) {
  const products = this.response.data.data;
...
Then('все товары в ответе имеют цену от {int} до {int} MDL', async function (this: World, minPrice: number, maxPrice: number) {
  const products = this.response.data.data;
...
Then('в ответе содержится информация о пагинации:', async function (this: World, dataTable) {
  const pagination = this.response.data.pagination;
...
Then('тело ответа содержит сообщение об ошибке валидации', async function (this: World) {
  const data = this.response.data;
  expect(data.error || data.message).toBeDefined();
...
Then('каждая категория содержит поля:', async function (this: World, dataTable) {
  const categories = this.response.data;
  const requiredFields = dataTable.hashes();
...
Given('существует товар с ID {string}', async function (this: World, productId: string) {
  // For tests, we'll assume the product exists or create test data
  this.testData = { ...this.testData, productId };
});

Given('существует категория {string} с ID {string}', async function (this: World, categoryName: string, categoryId: string) {
  // For tests, we'll assume the category exists or create test data
  this.testData = { ...this.testData, categoryName, categoryId };
});

Given('я отправил более {int} запросов за последнюю минуту', async function (this: World, requestCount: number) {

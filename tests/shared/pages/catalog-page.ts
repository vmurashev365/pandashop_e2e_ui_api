...
export class CatalogPage extends BasePage {
  ...
  private readonly selectors = {
    productGrid: '.content, .main, body, .page', // More generic container
    productImages: 'img', // Any image
    productPrices: '.price, [class*="price"], .cost, [class*="cost"], span:has-text("лей"), span:has-text("MDL")',
    productTitles: '.title, .name, [class*="title"], [class*="name"], h1, h2, h3, h4, a',
    productLinks: 'a[href*="/"], a',
    addToCartButtons: 'button:has-text("корзин"), button:has-text("cart"), .add-to-cart, button',
    categoryMenu: '.categories, .category-menu, .cat-nav, nav a',
    filterOptions: '.filter, .filters, .facet, select',
    sortOptions: '.sort, .sorting, .order-by, select',
    pagination: '.pagination, .pager, .page-nav, a:has-text("→"), a:has-text("»")',
